...
if (nrow(kegg_up) > 0) {
  ...
  p4 <- barplot(kegg_up, showCategory = 10, x = "GeneRatio", title = "KEGG Pathway - Upregulated") +
    scale_fill_gradient(low = "#0066CC", high = "#E5F0FF", 
                       name = "p.adjust",
                       guide = guide_colorbar(reverse = TRUE))
  print(p4)
  dev.off()
  cat("✓ アップレギュレーションKEGG解析図を保存\n")
}
...
cat("\n【アップレギュレーション - KEGGパスウェイ】\n")
if (nrow(kegg_up) > 0) {
  print(head(as.data.frame(kegg_up)[, c("Description", "GeneRatio", "p.adjust")], 10))
}
