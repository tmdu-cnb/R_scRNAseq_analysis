#!/usr/bin/env python3
"""
DT_PURKINJE_counts.csvに遺伝子IDを追加し、カウント列名をdta1, dta2, ... に変更するスクリプト
"""
...
def process_file(input_file, mapping_file, output_file):
    ...
    # 最初の2列: gene_id, gene
    # 残りの列: dta1, dta2, ...
    count_columns = header[1:]  # 元のgene列以降（空列を除外）
    # 空でない列のみをカウント
    count_columns = [col for col in count_columns if col.strip()]
    
    new_header = ['gene_id', 'gene']
    for i in range(len(count_columns)):
        new_header.append(f'dta{i+1}')
    ...
    with open(output_file, 'w', encoding='utf-8', newline='') as f:
        ...
        # 各行について、カウントデータ列のみを書き込み（空列は除外）
        for row in rows_sorted:
            new_row = row[:2]  # gene_id, gene
            # カウントデータ列を追加（元のgene列以降）
            if len(row) > 2:
                # 空でない列の数だけ追加
                count_data = row[2:2+len(count_columns)]
                new_row.extend(count_data)
